...
public interface StockMngMapper {
    @Select("select stock_tb.product_code as productCode, stock_tb.count as stockCount, name, price, sum(user_orders.count) as ownedCount, description\n"
    ...
    BigDecimal queryProductPrice(@Param("productCode") String productCode,
    ...
    @Insert("insert into stock_tb (product_code, name, description, price, count, user_name) values (#{productCode}, "
    ...
    void insertStockRecord(@Param("productCode") String productCode, @Param("name") String name,
                           ...
                           @Param("price") BigDecimal price, @Param("count") Integer count,
                           @Param("userName") String userName);

    @Insert("insert into order_tb (user_name, product_code, count) values (#{userName}, #{productCode}, #{count})")
    int createOrder(@Param("userName") String userName, @Param("productCode") String productCode,
                    @Param("count") int count);

    @Update("update stock_tb set count=count - #{count} where product_code=#{productCode} and user_name=#{userName}")
    int minusStockCount(@Param("userName") String userName,
                        @Param("productCode") String productCode, @Param("count") int count);
}
